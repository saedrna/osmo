...
uint8_t *camera_mode_switch_creator(const void *structure, size_t *data_length, uint8_t cmd_type) {
    ...
    // 判断是否为命令帧
    if ((cmd_type & 0x20) == 0) {
        const camera_mode_switch_command_frame_t *command_frame = (const camera_mode_switch_command_frame_t *)structure;
...
int version_query_parser(const uint8_t *data, size_t data_length, void *structure_out, uint8_t cmd_type) {
    ...
    if (data_length < fixed_length) {
        ESP_LOGE(TAG,
                 "version_query_parser: Data length too short for response frame. Expected at least: %zu, Got: %zu",
    ...
    ESP_LOGI(TAG, "Version Query Response parsed successfully. ack_result: %u, product_id: %s, sdk_version: %.*s",
             output_response->ack_result, output_response->product_id, (int)sdk_version_length,
             output_response->sdk_version);
...
uint8_t *record_control_creator(const void *structure, size_t *data_length, uint8_t cmd_type) {
    ...
    if ((cmd_type & 0x20) == 0) {
        const record_control_command_frame_t *command_frame = (const record_control_command_frame_t *)structure;
...
int connection_data_parser(const uint8_t *data, size_t data_length, void *structure_out, uint8_t cmd_type) {
    ...
    if ((cmd_type & 0x20) == 0) {
        ...
        if (data_length < sizeof(connection_request_command_frame)) {
            ESP_LOGE(TAG,
                     "connection_request_data_parser: Data length too short for command frame. Expected: %zu, Got: %zu",
        ...
        if (data_length < sizeof(connection_request_response_frame)) {
            ESP_LOGE(
                TAG,
                "connection_request_data_parser: Data length too short for response frame. Expected: %zu, Got: %zu",
...
int camera_status_push_data_parser(const uint8_t *data, size_t data_length, void *structure_out, uint8_t cmd_type) {
    ...
    if ((cmd_type & 0x20) == 0) {
        if (data_length < sizeof(camera_status_push_command_frame)) {
            ESP_LOGE(TAG,
                     "camera_status_push_data_parser: Data length too short for command frame. Expected: %zu, Got: %zu",
...
uint8_t *key_report_creator(const void *structure, size_t *data_length, uint8_t cmd_type) {
    ...
    if ((cmd_type & 0x20) == 0) {
        const key_report_command_frame_t *command_frame = (const key_report_command_frame_t *)structure;
